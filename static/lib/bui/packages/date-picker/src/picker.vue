<template>
  <b-input
    ...
    <i slot="icon"
      ...
      :class="[showClose ? 'b-icon-close' : triggerClass]"
...
export default {
  ...
  computed: {
    ...
    triggerClass() {
      return this.type.indexOf('time') !== -1 ? 'b-icon-time' : 'b-icon-date';
    },
  ...
  methods: {
    ...
    handleBlur() {
      this.$emit('blur', this);
      this.dispatch('BFormItem', 'el.form.blur');
    },
