<template>
  ...
  >
    <div class="b-submenu__title" ref="submenu-title" :style="paddingStyle">
      <slot name="title"></slot>
      <i :class="{
        'b-submenu__icon-arrow': true,
        'b-icon-arrow-down': rootMenu.mode === 'vertical',
        'b-icon-caret-bottom': rootMenu.mode === 'horizontal'
      }">
      </i>
    </div>
    <template v-if="rootMenu.mode === 'horizontal'">
      <transition name="b-zoom-in-top">
        <ul class="b-menu" v-show="opened"><slot></slot></ul>
      </transition>
    </template>
...
<script>
  ...
  module.exports = {
    ...
    methods: {
      ...
      handleClick() {
        this.dispatch('BMenu', 'submenu-click', this);
      },
      handleMouseenter() {
